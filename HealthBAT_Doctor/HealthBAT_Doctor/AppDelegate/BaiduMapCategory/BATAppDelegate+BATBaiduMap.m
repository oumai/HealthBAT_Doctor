...
- (void)bat_startBaiduMap {
    ...
    if (!self.mapManager) {
        ...
        if ([bundleID isEqualToString:@"com.KmHealthBAT.Doctor.Enterprise.app"]) {

            //健康BAT企业版
            BOOL ret = [self.mapManager start:@"a4rgsBNuDiXQ46htGZVTXuMbkbaEK07y"  generalDelegate:nil];
            if (!ret) {
                DDLogError(@"百度地图启动失败！");
            }
        }

        if ([bundleID isEqualToString:@"com.KmHealthBAT.Doctor.app"]) {
            //  AppStone版
            BOOL ret = [self.mapManager start:@"9s4XujawC8nV1HaGdb3m5PUbNzUVn5hZ"  generalDelegate:nil];
            if (!ret) {
                DDLogError(@"百度地图启动失败！");
            }
        }
