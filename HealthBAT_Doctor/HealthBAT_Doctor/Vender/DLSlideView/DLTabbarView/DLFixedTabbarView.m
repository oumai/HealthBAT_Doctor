...
- (void)commonInit{
    ...
    scrollView_.contentSize = CGSizeMake(0, 0);
    [self addSubview:scrollView_];
    
    trackView_ = [[UIImageView alloc] initWithFrame:CGRectMake(_bottomLineSpaceX, self.bounds.size.height-kTrackViewHeight-1, self.bounds.size.width-2*_bottomLineSpaceX, kTrackViewHeight)];
...
- (void)layoutTabbar{
    ...
    float trackX = width*self.selectedIndex+_bottomLineSpaceX;
    trackView_.frame = CGRectMake(trackX, trackView_.frame.origin.y, width-2*_bottomLineSpaceX, kTrackViewHeight);
}
...
- (void)switchingFrom:(NSInteger)fromIndex to:(NSInteger)toIndex percent:(float)percent{
    ...
    trackView_.frame = CGRectMake(trackX+_bottomLineSpaceX, trackView_.frame.origin.y, CGRectGetWidth(trackView_.bounds), CGRectGetHeight(trackView_.bounds));
}
