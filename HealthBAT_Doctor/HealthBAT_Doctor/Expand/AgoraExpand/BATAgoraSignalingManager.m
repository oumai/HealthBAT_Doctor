...
- (void)registerAgoraSignaling
{
    ...
    self.venderKey = @"1a1a51431ae040ef8b02ee4dd7a5ba82";
#elif TESTING
    ...
    self.venderKey = @"764a4f09065d453e9f21727a241db460";
    ...
    self.agoraSignaling = [AgoraAPI getInstanceWithoutMedia:self.venderKey];
...
- (void)login:(NSString *)account token:(NSString *)token deviceID:(NSString *)deviceID complete:(void (^)(BOOL))complete
{
    ...
    [self.agoraSignaling login:self.venderKey account:account token:token uid:0 deviceID:deviceID];
...
- (void)channelQueryUserNum:(NSString *)channelID complete:(void (^)(NSString *, AgoraEcode, int))complete
{
    [self.agoraSignaling channelQueryUserNum:channelID];
    
    
    self.agoraSignaling.onChannelQueryUserNumResult = ^(NSString *channelID, AgoraEcode ecode, int num) {
        //获取频道内的人数
        
        if (complete) {
            complete(channelID,ecode,num);
        }
