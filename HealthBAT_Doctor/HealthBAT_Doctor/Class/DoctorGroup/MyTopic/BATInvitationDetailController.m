...
//            contentCell.contentLb.text = self.model.Data.PostContent;
            ...
            contentCell.topicDetailPriseBlock = ^() {
        ...
        [cell configTopicData:self.replyModel.Data[indexPath.row]];
         ...
         TopicReplyData *data = self.replyModel.Data[indexPath.row];
        if (data.IsComplaint) {
...
//        };
        ...
        WEAK_SELF(self);
        cell.TopicreplyTableView.topicreplyCommentAction = ^(NSIndexPath *commentIndexPath,secondReplyData *comment,NSInteger parentLevelId){
            ...
            if (person.Data.AccountID == [comment.AccountID integerValue]) {
                return;
            }
            ...
            self.ParentLevelID = comment.ParentLevelID;
...
- (UITextField *)searchTF {
    ...
    if (!_searchTF) {
        ...
        _searchTF.frame = CGRectMake(10, 10, SCREEN_WIDTH - 20, 30);
