...
//    [self.headImageView sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@",courseCommentData.PhotoPath]] placeholderImage:[UIImage imageNamed:@"personalCenter_defaultAvator"]];
//    self.nameLabel.text = courseCommentData.AccountName;
//    self.timeLabel.text = [self getTimeStringFromDateString:courseCommentData.CreatedTime];
//    self.likeCountLabel.text = [NSString stringWithFormat:@"%ld",(long)courseCommentData.StarCount];
//    self.commentCountLabel.text = [NSString stringWithFormat:@"%ld",(long)courseCommentData.ReplyNum];
//    self.likeButton.selected = courseCommentData.IsFocus;
//    
//    if (courseCommentData.SubCourseReplyList.count > 0) {
//        
//        WEAK_SELF(self);
...
//    [self.replyTableView loadCommentsData:courseCommentData.SubCourseReplyList];
//    
...
- (void)configTopicData:(TopicReplyData *)courseCommentData
{
    ...
    self.commentCountLabel.text = [NSString stringWithFormat:@"%@",courseCommentData.ReplyNum];
   ...
   // self.replyTableView.parentLevelId = courseCommentData.ID;
    [self.TopicreplyTableView loadCommentsData:courseCommentData.secondReplyList];
...
//    }];
    ...
    [self setBottomBorderWithColor:UIColorFromHEX(0xeeeeee, 1) width:SCREEN_WIDTH height:0.5 leftOffset:10 rightOffset:10];
}
...
- (UIButton *)likeButton
{
    if (_likeButton == nil) {
        ...
        [_likeButton setBackgroundImage:[UIImage imageNamed:@"icon_dianzan_pre"] forState:UIControlStateSelected];
