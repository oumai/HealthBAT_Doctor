...
@interface BATSendDynamicViewController () <UITableViewDelegate,UITableViewDataSource,YYTextViewDelegate,BATAddPicTableViewCellDelegate,TZImagePickerControllerDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate>
...
//    [sendBarButtonItem setTintColor:BASE_COLOR];
    ...
    [sendBtn setFrame:CGRectMake(0, 0, 60, 40)];
    [sendBtn setTitle:@"发布" forState:UIControlStateNormal];
    [sendBtn setTitleColor:BLUE_COLOR forState:UIControlStateNormal];
...
- (void)requestSendGroupsDynamic
{
   ...
   NSArray *arr = [_dynamicContent componentsSeparatedByString:@"\n"];
    ...
    //    NSDictionary *param = @{@"DynamicContent":_dynamicContent,
    //                            @"GroupId":@(_groupID),
    //                            @"PostType":@(_groupDetailDynamicOpration),
    //                            @"Address":_address,
    //                            @"DynamicImg":[dynamicImg componentsJoinedByString:@","]};
    
//    NSDictionary *param = @{@"body":_dynamicContent,
//                            @"groupId":@(-1),
//                            @"postType":@(_groupDetailDynamicOpration),
//                            @"address":_address,
//                            @"imgUrl":[dynamicImg componentsJoinedByString:@","]};
    
    NSDictionary *param = @{@"TopicID":self.topicID,
                            @"Title":_TopicTitle,
                            @"PostContent":_dynamicContent,
                            @"ImageUrl":[dynamicImg componentsJoinedByString:@","]};
    [self showProgress];
    ...
    //    [HTTPTool requestWithURLString:@"/api/DynamicLoop/PostDynamicLoop" parameters:param type:kPOST success:^(id responseObject) {
    //        [self showText:@"发布成功"];
    //        self.navigationItem.rightBarButtonItem.enabled = YES;
    //
    //        //通知刷新界面
    //        [[NSNotificationCenter defaultCenter] postNotificationName:@"SEND_DYNAMIC_SUCCESS" object:self];
    //
    //        [self.navigationController popViewControllerAnimated:YES];
    //    } failure:^(NSError *error) {
    //        self.navigationItem.rightBarButtonItem.enabled = YES;
    ...
    //    [HTTPTool requestWithURLString:@"/api/DynamicLoop/WithFormData" parameters:param constructingBodyWithBlock:^(id<AFMultipartFormData> formData) {
    //        // 将本地的文件上传至服务器
